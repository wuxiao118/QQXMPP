...
import com.zyxb.qqxmpp.bean3.XMPPMessageQueueInfo;
...
//	private boolean isQueueThreadStarted = false;
	...
	private static XMPPEngine3 engine;
	private XMPPConnection mXMPPConnection;
	private Object obj = new Object();

	public static XMPPEngine3 getEngine() {
		return engine;
	}

	public static void setEngine(XMPPEngine3 engine) {
		XMPPEngine3.engine = engine;
	}

	public XMPPEngine3 getInstance(Context context) {
		if (engine == null) {
			synchronized (obj) {
				if (engine == null) {
					engine = new XMPPEngine3(context);
				}
			}
		}
	 ...
	 * @param context
	 */
	public XMPPEngine3(Context context) {
		mContext = context;
	...
	public XMPPEngine3(Context context, String host, boolean smackdebug,
					   boolean requireSsl) {
		mContext = context;
...
//				Const.SP_USERNAME, "");
//		dataEngine.setUser(dataEngine.findXMPPUserByName(username));
...
//							}
						...
						xmppMsg.setMsgType(DB3Columns.MESSAGE_TYPE_CONTACT);
						xmppMsg.setState(DB3Columns.MESSAGE_STATE_RECEIVED);
						XMPPMessageQueueInfo info = new XMPPMessageQueueInfo();
...
//
//		public QueueThread(DataEngine engine) {
//			this.engine = engine;
//		}
//
//		@Override
...
//						engine.addXMPPUser(getXMPPUser(rosterEntry));
//					}
//					break;
//				case QUEUE_TYPE_USER_DELETE:
//					for (String roster : rosters) {
//						RosterEntry rosterEntry = mRoster.getEntry(roster);
//						engine.deleteXMPPUser(rosterEntry.getUser());
//					}
//					break;
//				case QUEUE_TYPE_USER_UPDATE:
//					for (String roster : rosters) {
//						RosterEntry rosterEntry = mRoster.getEntry(roster);
//						engine.updateXMPPUser(getXMPPUser(rosterEntry));
//					}
//					break;
//			}
//
...
//			engine.addNewXMPPMessge(xmppMsg);
//
...
//					// 消息发送
//					engine.updateXMPPMessageState(account,
//							DB3Columns.MESSAGE_STATE_SENDED);
//					break;
//				case QUEUE_TYPE_MESSAGE_SEND_FAIL:
//					// 发送失败
//					engine.updateXMPPMessageState(account,
//							DB3Columns.MESSAGE_STATE_FAIL);
//					break;
...
//			user.setGroup(getGroup(rosterEntry.getGroups()));
//
//			Presence presence = mRoster.getPresence(rosterEntry.getUser());
//			user.setStatusMode(getStatusInt(presence));
//			user.setStatusMessage(presence.getStatus());
//
//			return user;
//		}
//
