...
public class DBInit {
    // 用户列表
    private static final DBUser[] users;
    // 用户分组
    private static final DBFriendGroup[] fgs;
    // 分组成员
    private static final DBFriendGroupMapping[] fgMappings;
    // 群
    private static final DBGroup[] groups;
    // 群成员
    private static final DBGroupMapping[] members;
    // 系统消息类型
    private static final DBSystemGroup[] sysgroups;
    // 消息
    private static final DBMessage[] messages;
    ...
    static {
        ...
        DBUser[][] fgusers = {{users[0], users[1], users[3], users[2]},
        ...
        fgMappings = new DBFriendGroupMapping[size];
        ...
        for (int i = 0; i < friendgs.length; i++) {
            for (int j = 0; j < fgstates[i].length; j++) {
                ...
                mapping = new DBFriendGroupMapping();
                ...
                // mapping.setmUser(friendgs[i].getmUser());
        ...
        String[][] groupDatas = {
                ...
                {"100000008", "计02A-2", "大学同学", null,
                        new Date(2006 - 1900, 7, 9).getTime() + "", "同学"}};
        groups = new DBGroup[groupDatas.length];
        DBGroup g = null;
        ...
        members = new DBGroupMapping[size];
        DBGroupMapping gm = null;
        k = 1000;
        int m = 0;
        for (int i = 0; i < memberstates.length; i++) {
            for (int j = 0; j < memberstates[i].length; j++) {
                String[] temp = memberstates[i][j].split(":");
                gm = new DBGroupMapping();
                gm.setAccount(k++ + "");
        ...
        sysgroups = new DBSystemGroup[sysnames.length];
        DBSystemGroup sysgroup = null;
        m = 10000;
        for (int i = 0; i < sysnames.length; i++) {
            sysgroup = new DBSystemGroup();
        ...
        String[][] msgcontents = {
                ...
                {"系统将于凌晨2点维护,给您带来的不便,敬请谅解",
                        DBColumns.MESSAGE_STATE_READED + "", "1434101370000",
                ...
                {"港姐首轮面试启动,佳丽抢镜", DBColumns.MESSAGE_STATE_RECEIVED + "",
                        "1434101370000", DBColumns.MESSAGE_TYPE_SYS + ""},
                {"港姐首轮面试启动,佳丽抢镜", DBColumns.MESSAGE_STATE_RECEIVED + "",
                        "1434101370000", DBColumns.MESSAGE_TYPE_SYS + ""},
                {"港姐首轮面试启动,佳丽抢镜", DBColumns.MESSAGE_STATE_RECEIVED + "",
                        "1434101370000", DBColumns.MESSAGE_TYPE_SYS + ""},
                ...
                {"[一个群有新消息]", DBColumns.MESSAGE_STATE_RECEIVED + "",
                        "1434101320000", DBColumns.MESSAGE_TYPE_SYS + ""},
                {"[一个群有新消息]", DBColumns.MESSAGE_STATE_RECEIVED + "",
                        "1434101320000", DBColumns.MESSAGE_TYPE_SYS + ""},
                {"[一个群有新消息]", DBColumns.MESSAGE_STATE_RECEIVED + "",
                        "1434101320000", DBColumns.MESSAGE_TYPE_SYS + ""},
                ...
                {"[空间未读精华]龙淑珍：我眼里有你，你眼里有谁?",
                        ...
                        "1434101270000", DBColumns.MESSAGE_TYPE_SYS + ""},
                {"[空间未读精华]龙淑珍：我眼里有你，你眼里有谁?",
                        DBColumns.MESSAGE_STATE_RECEIVED + "",
                        "1434101270000", DBColumns.MESSAGE_TYPE_SYS + ""},
                {"[空间未读精华]龙淑珍：我眼里有你，你眼里有谁?",
                ...
                {"财经新闻：以智慧激活财富,富人理财新手法",
                        DBColumns.MESSAGE_STATE_RECEIVED + "",
                        "1434101150000", DBColumns.MESSAGE_TYPE_SYS + ""},
                {"财经新闻：以智慧激活财富,富人理财新手法",
                        DBColumns.MESSAGE_STATE_RECEIVED + "",
                        "1434101150000", DBColumns.MESSAGE_TYPE_SYS + ""},
                {"财经新闻：以智慧激活财富,富人理财新手法",
                ...
                {"你点歌,我来唱", DBColumns.MESSAGE_STATE_READED + "",
                        "1434101370000", DBColumns.MESSAGE_TYPE_SYS + ""},
                {"你点歌,我来唱", DBColumns.MESSAGE_STATE_READED + "",
                        "1434101370000", DBColumns.MESSAGE_TYPE_SYS + ""},
                {"你点歌,我来唱", DBColumns.MESSAGE_STATE_READED + "",
                ...
                {"查看匿名悄悄话", DBColumns.MESSAGE_STATE_RECEIVED + "",
                        "1434100080000", DBColumns.MESSAGE_TYPE_SYS + ""},
                {"查看匿名悄悄话", DBColumns.MESSAGE_STATE_RECEIVED + "",
                        "1434100080000", DBColumns.MESSAGE_TYPE_SYS + ""},

                {"优惠|重返童年,吃喝玩乐,爽!", DBColumns.MESSAGE_STATE_RECEIVED + "",
                        "1434101080000", DBColumns.MESSAGE_TYPE_SYS + ""},
                {"优惠|重返童年,吃喝玩乐,爽!", DBColumns.MESSAGE_STATE_RECEIVED + "",
                        "1434101080000", DBColumns.MESSAGE_TYPE_SYS + ""},
                {"优惠|重返童年,吃喝玩乐,爽!", DBColumns.MESSAGE_STATE_RECEIVED + "",
                        "1434101080000", DBColumns.MESSAGE_TYPE_SYS + ""},
                ...
                {"朋友生日要到了,送个祝福吧", DBColumns.MESSAGE_STATE_RECEIVED + "",
                        "1434100780000", DBColumns.MESSAGE_TYPE_SYS + ""},
                {"朋友生日要到了,送个祝福吧", DBColumns.MESSAGE_STATE_RECEIVED + "",
                        "1434100780000", DBColumns.MESSAGE_TYPE_SYS + ""},

                // 群消息
                {"看看这个问题怎么解决？", DBColumns.MESSAGE_STATE_READED + "",
                        "1434101370000", DBColumns.MESSAGE_TYPE_GROUP + ""},
                {"什么问题?", DBColumns.MESSAGE_STATE_READED + "",
                        "1434101371000", DBColumns.MESSAGE_TYPE_GROUP + ""},
                {"??????", DBColumns.MESSAGE_STATE_READED + "",
                        "1434101371200", DBColumns.MESSAGE_TYPE_GROUP + ""},
                {"sharp定义了实线,但是显示不出来", DBColumns.MESSAGE_STATE_READED + "",
                        "1434101373000", DBColumns.MESSAGE_TYPE_GROUP + ""},
                {"这个没遇到过,我看看", DBColumns.MESSAGE_STATE_READED + "",
                        "1434101374000", DBColumns.MESSAGE_TYPE_GROUP + ""},
                {"上次貌似遇到过这个问题,我想想", DBColumns.MESSAGE_STATE_READED + "",
                        "1434101375000", DBColumns.MESSAGE_TYPE_GROUP + ""},
                {"想起来了,size的height必须大于stoke width,否则不显示",
                        DBColumns.MESSAGE_STATE_READED + "", "1434101376000",
                        DBColumns.MESSAGE_TYPE_GROUP + ""},
                {"哦哦,原来如此", DBColumns.MESSAGE_STATE_READED + "",
                        "1434101370000", DBColumns.MESSAGE_TYPE_GROUP + ""},
                {"学习了", DBColumns.MESSAGE_STATE_READED + "", "1434101378000",
                        DBColumns.MESSAGE_TYPE_GROUP + ""},
                {"前两天还遇到一个更操蛋的问题,低版本下圆角矩形,bottomLeft居然是右下",
                        DBColumns.MESSAGE_STATE_READED + "", "1434101379000",
                        DBColumns.MESSAGE_TYPE_GROUP + ""},
                {"是啊,以前也遇到过,真是日了狗了", DBColumns.MESSAGE_STATE_READED + "",
                        "1434101380000", DBColumns.MESSAGE_TYPE_GROUP + ""},
                {"居然会犯这种错误....", DBColumns.MESSAGE_STATE_READED + "",
                        "1434101380200", DBColumns.MESSAGE_TYPE_GROUP + ""},
                {"看见高手也有这么2B的时候,我也就放心了", DBColumns.MESSAGE_STATE_READED + "",
                        "1434101381000", DBColumns.MESSAGE_TYPE_GROUP + ""},
                {"[大笑]", DBColumns.MESSAGE_STATE_READED + "",
                        "1434101381500", DBColumns.MESSAGE_TYPE_GROUP + ""},
                {"[微笑]", DBColumns.MESSAGE_STATE_READED + "",
                        "1434101382500", DBColumns.MESSAGE_TYPE_GROUP + ""},

                // 聊天消息
                {"在不?", DBColumns.MESSAGE_STATE_READED + "", "1434101381000",
                        DBColumns.MESSAGE_TYPE_CONTACT + ""},
                {"在", DBColumns.MESSAGE_STATE_READED + "", "1434101382000",
                        DBColumns.MESSAGE_TYPE_CONTACT + ""},
                {"问你个事", DBColumns.MESSAGE_STATE_READED + "",
                        "1434101383000", DBColumns.MESSAGE_TYPE_CONTACT + ""},
                {"说", DBColumns.MESSAGE_STATE_READED + "", "1434101384000",
                        DBColumns.MESSAGE_TYPE_CONTACT + ""},
                {"有木有做过facebook登陆", DBColumns.MESSAGE_STATE_READED + "",
                        "1434101384500", DBColumns.MESSAGE_TYPE_CONTACT + ""},
                {"这个真木有,你问问小鹏吧,他貌似做过", DBColumns.MESSAGE_STATE_READED + "",
                        "1434101385000", DBColumns.MESSAGE_TYPE_CONTACT + ""},
                {"OK,[谢谢]", DBColumns.MESSAGE_STATE_READED + "",
                        "1434101386000", DBColumns.MESSAGE_TYPE_CONTACT + ""},
                {"帮我测试一下这个", DBColumns.MESSAGE_STATE_RECEIVED + "",
                        "1434101485000", DBColumns.MESSAGE_TYPE_CONTACT + ""},
                {"在不", DBColumns.MESSAGE_STATE_RECEIVED + "",
                        "1434101484000", DBColumns.MESSAGE_TYPE_CONTACT + ""}
        ...
        messages = new DBMessage[msgcontents.length];
        ...
        for (int i = 0; i < messages.length; i++) {
            ...
            switch (type) {
                case DBColumns.MESSAGE_TYPE_SYS:
                    message.setFromGroup((DBSystemGroup) fromtos[i][0]);
                    message.setTo((DBUser) fromtos[i][1]);
                    message.setAccount("sys-" + (k++));
                    break;
                case DBColumns.MESSAGE_TYPE_GROUP:
                    message.setFrom((DBUser) fromtos[i][0]);
                    message.setToGroup((DBGroup) fromtos[i][1]);
    ...
    public static void addUser(Context context) {
        DBUserDAO dao = DAOFactory.getDB3UserDAO(context);
    ...
    public static void addFriendGroup(Context context) {
        DBFriendGroupDAO dao = DAOFactory.getDB3FriendGroupDAO(context);
    ...
    public static void addFriendState(Context context) {
        DBFriendGroupMappingDAO dao = DAOFactory
    ...
    public static void addGroup(Context context) {
        DBGroupDAO dao = DAOFactory.getDB3GroupDAO(context);
    ...
    public static void addGroupState(Context context) {
        DBGroupMappingDAO dao = DAOFactory.getDB3GroupMappingDAO(context);
    ...
    public static void addSystemGroup(Context context) {
        DBSystemGroupDAO dao = DAOFactory.getDB3SystemGroupDAO(context);
    ...
    public static void addMessage(Context context) {
        DBMessageDAO dao = DAOFactory.getDB3MessageDAO(context);
        for (int i = 0; i < messages.length; i++) {
            dao.add(messages[i]);
        }
    ...
    public static void clear(Context context) {
        DBUserDAO userDao = DAOFactory.getDB3UserDAO(context);
        userDao.clear();
        userDao.close();

        DBFriendGroupDAO fgDao = DAOFactory.getDB3FriendGroupDAO(context);
        fgDao.clear();
        fgDao.close();

        DBFriendGroupMappingDAO fgmDao = DAOFactory
                .getDB3FriendGroupMappingDAO(context);
        fgmDao.clear();
        fgmDao.close();

        DBGroupDAO groupDao = DAOFactory.getDB3GroupDAO(context);
        groupDao.clear();
        groupDao.close();

        DBGroupMappingDAO gmDao = DAOFactory.getDB3GroupMappingDAO(context);
        gmDao.clear();
        gmDao.close();

        DBSystemGroupDAO sgDao = DAOFactory.getDB3SystemGroupDAO(context);
        sgDao.clear();
        sgDao.close();

        DBMessageDAO mDao = DAOFactory.getDB3MessageDAO(context);
        mDao.clear();
        mDao.close();
    }

    public static List<DBMessage> getGroupMessages(Context context) {
        DBMessageDAO dao = DAOFactory.getDB3MessageDAO(context);

        List<DBMessage> messages = dao.findGroup(users[0], groups[0]);
        dao.close();

        return messages;
    }

    public static List<DBMessage> getContactMessages(Context context) {
        DBMessageDAO dao = DAOFactory.getDB3MessageDAO(context);

        List<DBMessage> messages = dao.findContact(users[0], users[1]);
        dao.close();

        return messages;
    }

    public static List<DBMessage> getSystemMessages(Context context) {
        DBMessageDAO dao = DAOFactory.getDB3MessageDAO(context);

        List<DBMessage> messages = dao.findSystemGroup(users[0], sysgroups[0]);
        dao.close();

        return messages;
    }

    public static List<DBMessage> getNewest(Context context) {
        DBMessageDAO dao = DAOFactory.getDB3MessageDAO(context);

        List<DBMessage> messages = dao.findNewest(users[0]);
        dao.close();
